...
function TCell.init()
  ...
  imgGround= apiG.newImage('images//ground01.png') 
  imgPortals.blue = {
...
function TCell.newFood()
  ...
  food.storage = cfg.foodstorage  
  food.infinite = cfg.foodinfinite
  food.img = imgFood[3]
  food.isFound =false
  cfg.foodNumbers=cfg.foodNumbers+1
  cfg.foodAmount=cfg.foodAmount+food.storage
  --methods
function food.affectAnt( ant )
if food.isFound ==false then
  cfg.foodFoundNumbers=cfg.foodFoundNumbers+1
  food.isFound = true
end
...
function TCell.newCave()
  ...
  function cave.affectAnt( ant )
    if ant.lookingFor == cave.type then  
      cfg.foodArrivedStorage=cfg.foodArrivedStorage+ ant.cargo.count     
      ant.cargo.count = 0
      ant.maxTimeSeen = 0
      ant.taskFound(cave)
...
function TCell.newGround()
  local ground=TCell.newCell()
  ground.type = 'ground'
  ground.pass = true
  ground.img = imgGround
