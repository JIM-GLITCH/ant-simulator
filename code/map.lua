...
map.widthXg = map.maxXg-map.minXg+1
map.heightXg= map.maxYg-map.minYg+1
print(map.minXg,map.maxXg,map.minYg,map.maxYg )
...
function map.initCell(xg, yg)
    local x=xg-map.minXg+1
    local y =yg-map.minYg+1
    map.grid[xg][yg] = {
        qlist = TQuickList.create(),
        dcolor = {math.random(160), math.random(160), math.random(250)},
        pheromInfo = { seen = {} },
         --pasable or obstacle? setting borders 
        cell = mapExample.totype(x,y),
        pass = mapExample.topass(x,y),
        isFound =false,
        --ants communicate with others in the same grid
        ants={}
      }
...
function map.addAnt( ant )
  ...
  ant.id = map.ants.count
  ant.grid={math.floor(cfg.antBirthPlace[1]), 
            math.floor(cfg.antBirthPlace[2])}
  -- map.addActor(ant)
...
local cellPheromInfoClean = function(cell,i,j)
  cell.pheromInfo.seen={}
  
end

function map.draw()      
  apiG.print(map.widthXg.." x "..map.heightXg)  --
  for i = map.minXg, map.maxXg do
    for j = map.minYg, map.maxYg do
